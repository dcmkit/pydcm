...
def unpack_bits(ds):
    ...
    raw = ds.PixelData if hasattr(ds, "PixelData") else ds.get("PixelData")
    if raw is None:
        # NB: raise something OTHER than AttributeError — inside the pixel_array @property
        # an AttributeError is swallowed by __getattr__ and resurfaces as the misleading
        # "object has no attribute 'pixel_array'". Be explicit about the real cause.
        ts = str(ds.file_meta.get("TransferSyntaxUID") or "") if getattr(ds, "file_meta", None) else ""
        if ts in ("1.2.840.10008.1.2.2", "1.2.840.10008.1.2.1.99"):
            raise NotImplementedError(
                f"1-bit packed PixelData is not decodable for the {ts} (Big-Endian / "
                "Deflated) transfer syntax — transcode to a Little-Endian syntax first")
        raise ValueError("no PixelData to unpack")
    rows, cols = int(ds.get("Rows")), int(ds.get("Columns"))
    frames = int(ds.get("NumberOfFrames", 1) or 1)
    bits = np.unpackbits(np.frombuffer(bytes(raw), dtype="uint8"), bitorder="little")
    return bits[: frames * rows * cols].reshape(frames, rows, cols).astype("uint8")
...
def decode_uncompressed(ds):
    ...
    bs = int(ds.get("BitsStored", ba) or ba)
    signed = int(ds.get("PixelRepresentation", 0) or 0) == 1
    udt = {8: "u1", 16: "<u2", 32: "<u4"}.get(ba)
    if udt is None:
        raise NotImplementedError(f"BitsAllocated={ba} is not supported for in-memory decode")
    a = np.frombuffer(raw, dtype=udt)[: frames * rows * cols * spp]
    if signed:                                      # two's-complement sign-extend from BitsStored
        sign_bit = 1 << (bs - 1)
        a = ((a.astype("int64") & ((1 << bs) - 1)) ^ sign_bit) - sign_bit
        a = a.astype({8: "<i1", 16: "<i2", 32: "<i4"}[ba])
    if spp > 1:
        if int(ds.get("PlanarConfiguration", 0) or 0) == 0:    # interleaved RGBRGB…
            a = a.reshape(frames, rows, cols, spp)
        else:                                                   # planar RR…GG…BB…
            a = a.reshape(frames, spp, rows, cols).transpose(0, 2, 3, 1)
    else:
        a = a.reshape(frames, rows, cols)
...
def _decode_all(src):
    # 1-bit packed pixels: the native codec path treats them as 8-bit; unpack here.
    if hasattr(src, "get") and src.get("BitsAllocated") == 1:
        return unpack_bits(src)
    path, tmp = _as_path(src)
    try:
        arr, _ = _native.decode(path, 0, False)
        return arr
    finally:
        if tmp is not None:
            import os
...
def apply_color_lut(arr, ds=None, palette=None):
    ...
    def _lut(desc_tag, data_tag):
        desc = ds.get(desc_tag)
        ...
        first = int(desc[1])
        bits = int(desc[2])
        raw = data if not isinstance(data, (bytes, bytearray)) else None
        if raw is None:
            arr16 = np.frombuffer(bytes(data), dtype="<u2")
            vals = (arr16 >> 8).astype("uint8") if bits == 16 else arr16.astype("uint8")
        else:
            vals = np.asarray(raw, dtype="uint16")
            vals = (vals >> 8).astype("uint8") if bits == 16 else vals.astype("uint8")
        return vals[:n], first

    r, rf = _lut("RedPaletteColorLookupTableDescriptor", "RedPaletteColorLookupTableData")
    g, gf = _lut("GreenPaletteColorLookupTableDescriptor", "GreenPaletteColorLookupTableData")
    b, bf = _lut("BluePaletteColorLookupTableDescriptor", "BluePaletteColorLookupTableData")
    if r is None or g is None or b is None:
        return arr
    idx = arr.astype("int64")
    out = np.stack([r[np.clip(idx - rf, 0, len(r) - 1)],
                    g[np.clip(idx - gf, 0, len(g) - 1)],
                    b[np.clip(idx - bf, 0, len(b) - 1)]], axis=-1)
    return out
