...
pydcm ships no bundled corpus; this re-exports optional sample-data helpers when
available (so ``pydcm.data.get_testdata_file('CT_small.dcm')`` works in ported tests).
"""
...
def _backend():
    try:
        ...
        raise ModuleNotFoundError(
            "pydcm.data requires the optional sample-data provider to be installed"
        ) from e
