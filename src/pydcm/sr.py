    ...
    if reference is not None:
        ...
        for key, attr in (("patient_name", "PatientName"), ("patient_id", "PatientID"),
            ...
            if not doc[key]:
                ...
                if v not in (None, ""):
                    doc[key] = str(v)
    # This document's own identity. Empty keeps the engine's DETERMINISTIC
    # generator, which gives two documents built for one study the SAME UIDs —
    # right for a single self-contained export, a global-uniqueness violation
    # for a producer that mints many.
    if sop_instance_uid:
...
def read_regions(path):
    ...
    An importer cannot use that walk. Real producers put regions at five
    different depths and three of them carry no DCM 125007 anywhere in the
    chain, so a template-driven walk returns nothing for them. This enumerates
    every SCOORD / SCOORD3D regardless of ancestry — plus every image reference
    that names a segment, which is TID 1411's way of saying the region of
    interest **is** that segment — and gathers each one's context by looking
    around it.

    It is the same enumeration the native ROI importer drives, so a reader and
    an importer cannot disagree about what a document contains.
    ...
    """
    return _core.sr_regions(str(path))
