...
class UPS:
    ...
    QUERY = sop_class.UnifiedProcedureStepQuery
...
def _to_dataset(data: bytes, ts: str = _IMPLICIT_VR_LE,
    ...
    import os, tempfile
    part10 = _wrap_part10(data, ts, sop_class, sop_instance)
    fd, tmp = tempfile.mkstemp(suffix=".dcm", prefix="pydcm_dimse_")
    try:
        os.write(fd, part10)
        os.close(fd)
        ds = dcmread(tmp, force=True)
        # A received DIMSE data-set is transient: the temp file is unlinked below, so make the
        # data set wholly in-memory — otherwise a handler that re-encodes it (save_as / dcmwrite /
        # deepcopy, e.g. spooling a C-GET / C-MOVE result to disk, or load_series) re-reads the
        # deleted file and hits "cannot open <tmp>". While the file still exists: build file_meta
        # (the lazy property caches it — also keeps the received TransferSyntaxUID on
        # event.dataset.file_meta) and materialise the pixel element(s) for reading. Then keep the
        # ORIGINAL Part-10 bytes as the in-memory verbatim source so save_as preserves the exact
        # encoding incl. compressed (the from-scratch JSON writer can't), and drop the file path.
        _ = ds.file_meta
        for _kw in ("PixelData", "FloatPixelData", "DoubleFloatPixelData"):
            if _kw in ds:
                getattr(ds, _kw)
        object.__setattr__(ds, "_source_bytes", part10)
...
# Identifier-bearing query models — C-FIND/MOVE/GET for the three Q/R roots + Modality Worklist.
# Their identifier is always Implicit VR LE encoded (`_identifier_bytes` → encode_ivr), so propose
# IVR-only for these contexts: an SCP that picks Explicit VR LE (e.g. Orthanc) would otherwise be
# handed IVR bytes it can't decode and the C-FIND/MOVE/GET stalls. IVR LE is the mandatory Q/R
# baseline (PS3.4 §C.6.x), so this is fully conformant. Matches what the CLI's find/move/get SCUs
# already do (cli/src/find_scu.cpp). Storage / Verification keep IVR+EVR.
_QUERY_MODELS = {f"1.2.840.10008.5.1.4.1.2.{r}.{s}" for r in (1, 2, 3) for s in (1, 2, 3)} \
    | {"1.2.840.10008.5.1.4.31"}                                     # + Modality Worklist FIND


def _default_ts_for(abstract_syntax):
    """The default proposed transfer syntaxes for a context with no explicit list: IVR-only for
    query models (so the negotiated TS matches the IVR-encoded identifier), IVR+EVR otherwise."""
    return ["1.2.840.10008.1.2"] if str(abstract_syntax) in _QUERY_MODELS else list(_DEFAULT_TS)
# The curated common image/report-Storage classes a C-GET SCU offers by default, taken from the
# native engine's single source (no Python-side hand copy to drift).
_COMMON_STORAGE = list(_dimse.common_storage_classes())
# (abstract_syntax, [transfer_syntaxes]) — Verification + Q/R FIND/MOVE + common storage. Query
# models get IVR-only (see _default_ts_for); Verification + storage keep IVR+EVR.
_DEFAULT_CONTEXTS = [(uid, _default_ts_for(uid))
...
def _normalise_contexts(contexts):
    if contexts is None:
        return None
    out = []
    for ctx in contexts:
        if hasattr(ctx, "abstract_syntax"):
            abstract = getattr(ctx, "abstract_syntax")
            transfer_syntax = getattr(ctx, "transfer_syntax", None)
        elif isinstance(ctx, (tuple, list)) and ctx:
            abstract = ctx[0]
            transfer_syntax = ctx[1] if len(ctx) > 1 else None
        else:
            abstract = ctx
            transfer_syntax = None

        if transfer_syntax is None or transfer_syntax == []:
            ts = _default_ts_for(abstract)
        elif isinstance(transfer_syntax, str):
            ts = [transfer_syntax]
        else:
            ts = [str(t) for t in transfer_syntax]
        out.append((str(abstract), ts))
    return out
...
def _reject_unsupported_ext_neg(ext_neg):
    if ext_neg:
        raise NotImplementedError(
            "pydcm.dimse does not yet implement ext_neg objects; "
            "configure role/context support with add_supported_context() or "
            "contexts= for the supported common workflows")


def _looks_like_evt_handlers(value):
    if not isinstance(value, (tuple, list)):
        return False
    if not value:
        return True
    first = value[0]
    return isinstance(first, (tuple, list)) and first and hasattr(first[0], "name")


# --------------------------------------------------------------------------- #
#  Association (persistent — one negotiate, many ops, one release)
# --------------------------------------------------------------------------- #
def _split_bind_address(bind_address):
    if bind_address is None:
        return "", 0
    if isinstance(bind_address, (tuple, list)):
        if not bind_address:
            return "", 0
        host = "" if bind_address[0] is None else str(bind_address[0])
        port = int(bind_address[1]) if len(bind_address) > 1 and bind_address[1] is not None else 0
        return host, port
    return str(bind_address), 0
...
class Association:
    ...
    def __init__(self, ae, host, port, called_ae, n_event_handler=None, lifecycle=None,
                 bind_address=None, contexts=None, max_pdu=None):
        self._ae = ae
        self._host, self._port, self._called = host, int(port), called_ae
        self._bind_address = bind_address
        self._thread_name = f"pydcm.dimse:{ae.ae_title}->{called_ae}"
        self._thread_daemon = True
        ...
        proposed_contexts = contexts if contexts is not None else (ae._requested or _DEFAULT_CONTEXTS)
        contexts, seen = [], set()
        for abstract, ts in proposed_contexts:
        ...
        if n_event_handler is not None:
            def native_event_cb(sc, si, et, info, abstract_syntax, _h=n_event_handler):
                return _norm_n_return(_h(_NEvent(
                    sc, si, dataset=_reply_dataset(info), type_id=et,
                    abstract_syntax_uid=abstract_syntax or None)))
        ...
        self._get_storage = [abstract for abstract, _ts, _scu_role, scp_role in ae._supported
                             if scp_role is not False]
        try:
            source_host, source_port = _split_bind_address(bind_address)
            self._client = _dimse.client_connect(
                host=host, port=int(port), calling_ae=ae.ae_title, called_ae=called_ae,
                contexts=contexts,
                max_pdu=int(ae.maximum_pdu_size if max_pdu is None else max_pdu),
                tls=ae.tls, on_n_event_report=native_event_cb,
    ...
    def local(self):
        ...
        host, port = _split_bind_address(self._bind_address)
        return {"ae_title": self._ae.ae_title, "address": host, "port": port}
    ...
    @property
    def name(self):
        """Thread-compatible association name property."""
        return self._thread_name

    @name.setter
    def name(self, value):
        self._thread_name = str(value)

    @property
    def daemon(self):
        """Thread-compatible daemon flag. pydcm has no Python association thread."""
        return self._thread_daemon

    @daemon.setter
    def daemon(self, value):
        self._thread_daemon = bool(value)
    ...
    def is_alive(self):
        return bool(self.is_established)
    ...
    def get_handlers(self, event):
        ...
        handler = self._lifecycle.get(key)
        return [] if handler is None else [(handler, None)]
    ...
    def run_reactor(self):
        raise NotImplementedError(
            "pydcm.dimse associations are native requestor associations; "
            "the Python reactor loop is not exposed")

    def set_socket(self, socket):
        raise NotImplementedError(
            "pydcm.dimse owns its native socket; set_socket() is an "
            "acceptor-internal hook")
    ...
    def send_c_cancel(self, msg_id, context_id=None, query_model=None):
    ...
    def _common(self):
        ...
        source_host, source_port = _split_bind_address(self._bind_address)
        return dict(host=self._host, port=self._port, calling_ae=self._ae.ae_title,
                    called_ae=self._called, tls=self._ae.tls,
                    source_host=source_host, source_port=source_port,
                    timeout_ms=self._ae._establish_timeout_ms(),
                    dimse_timeout_ms=(int(self._ae.dimse_timeout * 1000)
                                      if self._ae.dimse_timeout else 0))
    ...
    def send_c_store(self, dataset, msg_id=1, priority=2, originator_aet=None,
        ...
        self._require()
        if isinstance(priority, str) and originator_aet is None:
            originator_aet, priority = priority, 2
        if isinstance(dataset, (str, bytes, os.PathLike)):
            return self.send_c_store_file(dataset, msg_id=msg_id,
    ...
    @_empty_on_net_error()
    def send_c_store_file(self, path, msg_id=1, originator_aet=None,
                          originator_id=None, context_id=None):
        """C-STORE one Part-10 file without materialising it as a Python Dataset.

        This is the file-path fast path for large studies: native code reads
        only the Part-10 file meta up front, then streams the bare data set
        bytes from disk through the native streaming C-STORE API.
        """
        self._require()
        r = self._client.store_files([os.fspath(path)])
        if r["succeeded"] == 0 and r["sent"] > 0:
            return _status(r["last_status"] or 0xA700)
        return _status(r["last_status"])

    def send_c_store_files(self, paths):
        ...
        """
        self._require()
        return self._client.store_files([os.fspath(p) for p in paths])
    ...
    @_empty_gen_on_net_error
    def send_c_get(self, dataset, query_model, msg_id=1, priority=2, store_dir=None):
        ...
        ident = _identifier_bytes(dataset)
        if store_dir:
            if not self._get_storage:
                raise RuntimeError("native C-GET to dir needs add_supported_context() for the "
                                   "storage classes (so the SCP role is negotiated at associate())")
            r = self._client.get(str(query_model), ident, None, str(store_dir))
            yield _status_with_counts(r), None
            return
        ...
        else:                        # one-shot fallback (caller didn't add_supported_context):
            # the common image-Storage classes — same curated set the persistent default contexts
            # and the CLI get-scu use (best effort; declare via add_supported_context for a study
            # with uncommon classes, since a non-transcoding SCP aborts the C-GET on any matched
            # class without a negotiated context).
            r = _dimse.get(model=_model_of(query_model), identifier=ident,
                           accept_storage_classes=list(_COMMON_STORAGE), on_store=on_store, **self._common)
        yield _status_with_counts(r), None
    ...
    def send_n_create(self, dataset, class_uid, instance_uid=None, msg_id=1, meta_uid=None):
        ...
        r = self._client.n_create(str(class_uid), str(instance_uid) if instance_uid else None,
                                  attrs, int(msg_id), str(meta_uid or ""))
        st = _status(r["status"])
    ...
    def ups_push(self, workitem, instance_uid=None):
        ...
        status, reply = self.send_n_create(
            workitem, UPS.PUSH, instance_uid=instance_uid, meta_uid=UPS.PUSH)
        if status is not None and not getattr(status, "AffectedSOPInstanceUID", None):
            status.AffectedSOPInstanceUID = instance_uid
        return status, reply

    def ups_get(self, workitem_uid, attributes=None):
        """UPS Pull: read a workitem (N-GET); ``attributes`` empty => whole workitem."""
        return self.send_n_get(attributes or [], UPS.PUSH, workitem_uid,
                               meta_uid=UPS.PULL)
    ...
    def ups_set(self, workitem_uid, modifications, transaction_uid):
        ...
        return self.send_n_set(modifications, UPS.PUSH, workitem_uid,
                               meta_uid=UPS.PULL)
    ...
    def ups_change_state(self, workitem_uid, state, transaction_uid):
        ...
        return self.send_n_action(ds, UPS.CHANGE_STATE, UPS.PUSH, workitem_uid,
                                  meta_uid=UPS.PULL)
    ...
    def ups_request_cancel(self, workitem_uid, reason=None):
        """UPS Push: request cancellation of a workitem you don't own (N-ACTION
        type 2); optional (0074,1238) Reason For Cancellation."""
        ds = None
        if reason is not None:
            ds = Dataset()
            ds.ReasonForCancellation = reason
        return self.send_n_action(ds, UPS.REQUEST_CANCEL, UPS.PUSH, workitem_uid,
                                  meta_uid=UPS.PUSH)
    ...
    def ups_subscribe(self, target=None, receiving_ae=None):
        ...
        return self.send_n_action(ds, UPS.SUBSCRIBE, UPS.PUSH,
    ...
    def ups_unsubscribe(self, target=None, receiving_ae=None):
        ...
        if receiving_ae is not None:
            ds = Dataset()
            ds.ReceivingAE = receiving_ae
        return self.send_n_action(ds, UPS.UNSUBSCRIBE, UPS.PUSH,
                                  target or UPS.GLOBAL_SUBSCRIPTION,
                                  meta_uid=UPS.WATCH)
...
class _NEvent:
    ...
    def __init__(self, sop_class, sop_instance, *, dataset=None, type_id=None,
                 identifiers=None, abstract_syntax_uid=None):
        self.sop_class_uid = sop_class
        self.sop_instance_uid = sop_instance
        self.abstract_syntax_uid = abstract_syntax_uid
        self._dataset = dataset
...
class AE:
    ...
    def remove_supported_context(self, abstract_syntax, transfer_syntax=None):
        ...
        if transfer_syntax is None:
            self._supported = [ctx for ctx in self._supported if ctx[0] != a]
            return
        remove_ts = ([str(transfer_syntax)] if isinstance(transfer_syntax, str)
                     else [str(t) for t in transfer_syntax])
        remaining = []
        for abstract, ts, scu_role, scp_role in self._supported:
            if abstract != a:
                remaining.append((abstract, ts, scu_role, scp_role))
                continue
            kept_ts = [t for t in ts if t not in remove_ts]
            if kept_ts:
                remaining.append((abstract, kept_ts, scu_role, scp_role))
        self._supported = remaining

    def make_server(self, address, ae_title=None, contexts=None, ssl_context=None,
                    evt_handlers=None, server_class=None, **kw):
        """Return a non-blocking SCP server handle; call
        ``.serve_forever()`` / ``.shutdown()`` on it. ``== start_server(block=False)``."""
        if _looks_like_evt_handlers(ssl_context):
            evt_handlers, ssl_context = ssl_context, evt_handlers
        if server_class is not None:
            raise NotImplementedError(
                "pydcm.dimse uses its native ServerHandle and does not support "
                "custom server_class")
        return self.start_server(address, block=False, ssl_context=ssl_context,
                                 evt_handlers=evt_handlers, ae_title=ae_title,
    ...
    def add_requested_context(self, abstract_syntax, transfer_syntax=None):
        if transfer_syntax is None:
            ts = _default_ts_for(abstract_syntax)       # IVR-only for query models, else IVR+EVR
        elif isinstance(transfer_syntax, str):
            ts = [transfer_syntax]
        else:
            ts = [str(t) for t in transfer_syntax]
    ...
    def add_requested_contexts_for_files(self, paths):
        ...
        """
        for sop_class, transfer_syntax in _dimse.store_file_contexts([os.fspath(p) for p in paths]):
            ctx = (str(sop_class), [str(transfer_syntax)])
            if ctx not in self._requested:
                self._requested.append(ctx)

    def add_supported_context(self, abstract_syntax, transfer_syntax=None,
                              scu_role=None, scp_role=None):
        abstract, ts = _normalise_contexts([(abstract_syntax, transfer_syntax)])[0]
        scu_role = _normalise_role(scu_role)
        scp_role = _normalise_role(scp_role)
        for idx, (existing_abstract, existing_ts, _scu_role, _scp_role) in enumerate(self._supported):
            if existing_abstract != abstract:
                continue
            merged_ts = list(existing_ts)
            for item in ts:
                if item not in merged_ts:
                    merged_ts.append(item)
            self._supported[idx] = (abstract, merged_ts, scu_role, scp_role)
            return
        self._supported.append((abstract, ts, scu_role, scp_role))
    ...
    def supported_contexts(self):
        return [PresentationContext(abstract_syntax=a, transfer_syntax=list(ts),
                                    scu_role=scu_role, scp_role=scp_role)
                for a, ts, scu_role, scp_role in self._supported]
    ...
    def supported_contexts(self, contexts):
        ...
        for ctx in contexts:
            self.add_supported_context(
                getattr(ctx, "abstract_syntax", ctx),
                getattr(ctx, "transfer_syntax", None),
                getattr(ctx, "scu_role", None),
                getattr(ctx, "scp_role", None))

    def _get_store_handler(self):
        return self._store_handler

    def associate(self, addr, port, contexts=None, ae_title="ANY-SCP", max_pdu=None,
                  ext_neg=None, bind_address=None, tls_args=None, evt_handlers=None, **_kw):
        # pydcm historically used the third positional argument for ae_title.
        # Common SCU call style uses it for contexts. Keep the old string form
        # working while accepting a contexts list.
        if isinstance(contexts, str):
            if ae_title == "ANY-SCP":
                ae_title, contexts = contexts, None
            elif not isinstance(ae_title, str):
                ae_title, contexts = contexts, ae_title
        if bind_address is None:
            bind_address = _kw.get("bind_address")
        if max_pdu is None:
            max_pdu = _kw.get("max_pdu", self.maximum_pdu_size)
        _reject_unsupported_ext_neg(ext_neg if ext_neg is not None else _kw.get("ext_neg"))
        requested_contexts = _normalise_contexts(contexts)
        ...
        assoc = Association(self, addr, port, str(ae_title),
                            ...
                            contexts=requested_contexts, max_pdu=max_pdu)
        self._active.append(assoc)
        return assoc

    def get_to_dir(self, addr, port, dataset, query_model, store_dir,
                   ae_title="ANY-SCP", tls_args=None, accept_storage_classes=None,
                   bind_address=None):
        ...
        self.add_requested_context(query_model, transfer_syntax=[_IMPLICIT_VR_LE])
        for cx in (accept_storage_classes or StoragePresentationContexts[:127]):
            self.add_supported_context(getattr(cx, "abstract_syntax", cx))
        assoc = self.associate(str(addr), int(port), ae_title=str(ae_title),
                               tls_args=tls_args, bind_address=bind_address)
        try:
            if not assoc.is_established:
                raise RuntimeError("association rejected, aborted, or unreachable")
            result = None
            for status, _ in assoc.send_c_get(dataset, query_model, store_dir=str(store_dir)):
                result = status
            return {"status": int(getattr(result, "Status", 0)) if result is not None else 0}
        finally:
            if assoc.is_established:
                assoc.release()

    def start_get_to_dir(self, addr, port, dataset, query_model, store_dir,
                         ae_title="ANY-SCP", tls_args=None, accept_storage_classes=None,
                         bind_address=None):
        ...
        classes = [getattr(cx, "abstract_syntax", cx)
                   for cx in (accept_storage_classes or StoragePresentationContexts[:127])]
        source_host, source_port = _split_bind_address(bind_address)
        return _dimse.start_get_to_dir(
            host=str(addr), port=int(port), calling_ae=self.ae_title, called_ae=str(ae_title),
            model=str(query_model), identifier=_identifier_bytes(dataset),
            get_storage_classes=classes, store_dir=str(store_dir),
            tls=_tls_from_args(tls_args) if tls_args else None,
            source_host=source_host, source_port=source_port,
            timeout_ms=self._establish_timeout_ms(),
            dimse_timeout_ms=(int(self.dimse_timeout * 1000)
                              if self.dimse_timeout else 0))

    def start_server(self, address, block=True, ssl_context=None, evt_handlers=None,
                     ae_title=None, contexts=None, store_dir=None, **_kw):
        ...
        if _looks_like_evt_handlers(ssl_context):
            evt_handlers, ssl_context = ssl_context, evt_handlers
        host, port = address
        server_ae_title = self.ae_title if ae_title is None else str(ae_title)
        supported_contexts = _normalise_contexts(contexts)
        if supported_contexts is None and self._supported:
            supported_contexts = [(uid, list(ts)) for uid, ts, _scu_role, _scp_role
                                  in self._supported]
        ...
        for entry in (evt_handlers or []):
            ...
            # --- DIMSE-N SCP handlers (Normalized services) ---
                ...
                def on_n_action(sc, si, at, info, abstract_syntax, _h=handler):
                    return _norm_n_return(_h(_NEvent(
                        sc, si, dataset=_reply_dataset(info), type_id=at,
                        abstract_syntax_uid=abstract_syntax or None)))
            elif ev is evt.EVT_N_CREATE:
                def on_n_create(sc, si, attrs, abstract_syntax, _h=handler):
                    return _norm_n_return(_h(_NEvent(
                        sc, si, dataset=_reply_dataset(attrs),
                        abstract_syntax_uid=abstract_syntax or None)))
            elif ev is evt.EVT_N_SET:
                def on_n_set(sc, si, mods, abstract_syntax, _h=handler):
                    return _norm_n_return(_h(_NEvent(
                        sc, si, dataset=_reply_dataset(mods),
                        abstract_syntax_uid=abstract_syntax or None)))
            elif ev is evt.EVT_N_GET:
                def on_n_get(sc, si, tags, abstract_syntax, _h=handler):
                    return _norm_n_return(_h(_NEvent(
                        sc, si, identifiers=[tuple(t) for t in tags],
                        abstract_syntax_uid=abstract_syntax or None)))
            elif ev is evt.EVT_N_DELETE:
                def on_n_delete(sc, si, abstract_syntax, _h=handler):
                    return _norm_status(_h(_NEvent(
                        sc, si, abstract_syntax_uid=abstract_syntax or None)))
            elif ev is evt.EVT_N_EVENT_REPORT:
                def on_n_event(sc, si, et, info, abstract_syntax, _h=handler):
                    return _norm_n_return(_h(_NEvent(
                        sc, si, dataset=_reply_dataset(info), type_id=et,
                        abstract_syntax_uid=abstract_syntax or None)))
        tls = self.tls
        if ssl_context is not None:
            if not isinstance(ssl_context, dict):
                raise NotImplementedError(
                    "pydcm.dimse.start_server does not accept a Python ssl.SSLContext; "
                    "use ae.tls or ssl_context={...} with pydcm TLS settings")
            tls = ssl_context
        native = _dimse.server_start(port=int(port), ae_title=server_ae_title,
                                     ...
                                     tls=tls, store_dir=str(store_dir or ""),
...
def print_image(host, port, pixel_bytes, rows, cols,
                ...
                tls=None, connect_timeout=30, bind_address=None):
    ...
    bind_address
        Optional local source address for multihomed/VPN clients. Use
        ``("VPN_IP", 0)`` to let the OS assign the source port.
    ...
    assoc = ae.associate(host, port, ae_title=called_ae, bind_address=bind_address)
    if not assoc.is_established:
