...
def _empty_on_net_error(*, reply=False):
    """Map a transport/timeout/abort ConnectionError from a native DIMSE op to
    pynetdicom's empty-``Dataset`` result. A USAGE error (no negotiated context /
    association not established / bad encoding) still raises, as in pynetdicom.
...
def _empty_gen_on_net_error(fn):
    ...
    becomes one final ``(Dataset(), None)`` yield and the stream ends — matching how
    pynetdicom surfaces a failed/aborted query."""
    @functools.wraps(fn)
    def wrap(*args, **kwargs):
        try:
...
class evt:                                          # noqa: N801 (mirror pynetdicom.evt)
...
class AE:
    ...
    def __init__(self, ae_title="PYDCM"):
        ...
        self.tls = None                              # dict: ca_file/cert_file/key_file/...
    ...
    def implementation_class_uid(self):
        return PYNETDICOM_IMPLEMENTATION_UID
    ...
    def implementation_version_name(self):
        return PYNETDICOM_IMPLEMENTATION_VERSION
...
PYNETDICOM_UID_PREFIX = "1.2.826.0.1.3680043.9.7155."     # pydcm's arc
PYNETDICOM_IMPLEMENTATION_VERSION = "PYDCM_DIMSE_1"
PYNETDICOM_IMPLEMENTATION_UID = UID(PYNETDICOM_UID_PREFIX + "1")
...
class SCP_SCU_RoleSelection:                          # noqa: N801 (mirror pynetdicom)
