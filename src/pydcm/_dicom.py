...
def _build(jmap: dict, path=None) -> "Dataset":
    ...
    return Dataset(elements, path=path)
...
class DataElement:
    ...
    def description(self) -> str:
        """The element's name (pydicom ``DataElement.description()``)."""
        return self.name
...
class Dataset:
    ...
    __slots__ = ("_dict", "_path", "_pixels", "_file_meta", "_edits")
    ...
    def __init__(self, elements=None, *, path=None):
        object.__setattr__(self, "_dict", dict(elements or {}))
    ...
    def __getattr__(self, name: str):
        ...
        try:
            ...
            if tag in _PIXEL_DATA_TAGS and self._path is not None:
                ...
                if raw is not None:
                    el = DataElement(Tag(tag), _pixel_data_vr(self._path, self), raw,
    ...
    def pixel_array(self):
        ...
        axis. For Hounsfield units use :func:`pydcm.decode`
        with ``rescale=True`` or ``apply_modality_lut``.
        """
        if self._pixels is None:
            if self.get("BitsAllocated") == 1:
                # 1-bit packed: unpack the whole PixelData buffer then reshape — the only
                # path that's correct when frames are NOT byte-aligned (rows*cols % 8 != 0).
                # If the raw bytes aren't reachable (Big-Endian fast-path skip), fall back to
                # the native decoder, which unpacks byte-aligned 1-bit incl. Big-Endian.
                from .pixels import unpack_bits
                try:
                    arr = unpack_bits(self)
                except (NotImplementedError, ValueError, AttributeError):
                    if self._path is None:
                        raise
                    arr, _ = _native.decode(self._path, 0, False)
            elif self._path is None:
                # From-scratch dataset (no backing file). Uncompressed little-endian is a
                # pure-numpy buffer reinterpretation (robust — needs only the Image Pixel
                # tags); compressed / Big-Endian / Deflated are serialised to in-memory
                # Part-10 bytes and decoded by the SAME native engine as a file.
                if "PixelData" not in self:
                    raise AttributeError("this Dataset has no backing file (no PixelData)")
                ts = str(self.file_meta.get("TransferSyntaxUID", "")) \
                    if getattr(self, "file_meta", None) else ""
                # PALETTE COLOR is expanded to RGB by the native engine (pydcm's deliberate
                # divergence) — route it (+ compressed / Big-Endian / Deflated) through the
                # native decoder so in-memory matches the file path; plain uncompressed LE
                # grayscale/RGB/YBR is a fast pure-numpy reinterpret.
                palette = str(self.get("PhotometricInterpretation", "")).startswith("PALETTE")
                if ts in ("", "1.2.840.10008.1.2", "1.2.840.10008.1.2.1") and not palette:
                    from .pixels import decode_uncompressed
                    arr = decode_uncompressed(self)
                else:
                    arr, _ = _native.decode_bytes(self._encode_part10(), 0, False)
            else:
                arr, _ = _native.decode(self._path, 0, False)
    ...
    def set_pixel_data(self, arr, photometric_interpretation, bits_stored,
        ...
        """Set PixelData and the related Image Pixel module elements from ``arr`` (pydicom 3.x)."""
        import numpy as np
        a = np.asarray(arr)
    ...
    def compress(self, transfer_syntax_uid, arr=None, *, encoding_plugin="",
                 generate_instance_uid=True, **_kw) -> None:
        """Compress PixelData to ``transfer_syntax_uid`` in place (pydicom 3.x).

        Re-encodes via the native transcoder. Supported targets are the encapsulated
        lossless syntaxes — RLE Lossless (1.2.840.10008.1.2.5), JPEG 2000 Lossless (.4.90),
        JPEG-LS Lossless (.4.80), HTJ2K Lossless (.4.201), JPEG-XL. ``arr`` (optional)
        replaces the pixels first. Raises :class:`ValueError` for an unsupported target
        (e.g. a lossy syntax).
        """
        ...
        try:
            out = _native.transcode(self._encode_part10(), ts)
        except Exception as exc:
            raise ValueError(
                f"cannot compress to {ts}: only encapsulated lossless targets "
                "(JPEG2000 .90 / JPEG-LS .80 / HTJ2K .201 / JPEG-XL) are supported "
                f"[{exc}]") from exc
    ...
    def decompress(self, *args, **kwargs) -> None:
        """Decode a compressed PixelData to native little-endian *in place*. Reuses the native decoder — no extra machinery. ``ds.PixelData``
        then holds the native bytes and the Transfer Syntax becomes Explicit VR LE.
        (Re-serialising a decompressed dataset to a NEW file is the JSON-write path;
        the byte-verbatim editor only patches the original, still-compressed bytes.)"""
        arr = self.pixel_array
        self._dict[Tag(0x7FE00010)] = DataElement(
            Tag(0x7FE00010), "OW" if arr.dtype.itemsize > 1 else "OB",
            arr.tobytes(), keyword="PixelData")
    ...
    def _encode_part10(self, target_ts: str = "") -> bytes:
        ...
        recode = False
        if self._path is not None and target_ts in ("1.2.840.10008.1.2", "1.2.840.10008.1.2.1"):
            try:
        ...
        if self._path is not None and not be_or_deflated and not nested and not recode:
            try:
                with open(self._path, "rb") as f:
                    original = f.read()
                return _native.edit_part10(original, self._edit_ops())
            except Exception:
                pass                 # unexpected (malformed LE) — try the model path below
    ...
    def _requested_ts(self, implicit_vr, little_endian) -> str:
        ...
        TransferSyntaxUID. Only the two uncompressed VR-LE forms are honoured by the
        from-scratch writer; anything else (incl. EVR-BE, which the recoder doesn't do)
        falls back to ``""`` = the writer default (Implicit VR LE)."""
        if implicit_vr is not None:
            if little_endian is False:
                return ""                                  # EVR-BE not supported here
            return "1.2.840.10008.1.2" if implicit_vr else "1.2.840.10008.1.2.1"
        ts = str(self.file_meta.get("TransferSyntaxUID") or "") if self.file_meta is not None else ""
        return ts if ts in ("1.2.840.10008.1.2", "1.2.840.10008.1.2.1") else ""

    def save_as(self, filename, write_like_original=None, *,
                enforce_file_format=None, implicit_vr=None, little_endian=None,
                overwrite=True, **_ignored) -> None:
    ...
    # -- display (pydicom Dataset._pretty_str layout) ------------------------ #
...
def dcmread(fp, defer_size=None, stop_before_pixels=False, force=False,
            specific_tags=None, *, charset_override: str = "", **_ignored) -> Dataset:
    ...
    # (pydicom does the same), regardless of how the bytes were typed on the wire —
    ...
    # Lazy PixelData (7FE0,0010) is now built directly from read_json's value-less VR stub
    # (present in the mapping protocol — 'PixelData' in ds, len, iter, keys, ds[tag] — without
    # loading the bytes). For in-memory (tmp) input the backing file is
    # deleted in the finally above, so drop it (can't lazy-load) — preserving prior behavior.
    if tmp is not None:
        for _pt in _PIXEL_DATA_TAGS:
            ds._dict.pop(Tag(_pt), None)

    # Return a FileDataset (pydicom contract) — gives .filename/.preamble/.timestamp
...
def dcmwrite(path, dataset: Dataset, **_ignored) -> None:
