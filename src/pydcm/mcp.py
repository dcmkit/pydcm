...
pydcm end to end — analysis (radiomics / DVH / validation), perfusion (DCE + DSC),
...
Every tool reads on the same foundation: ALL image codecs / transfer syntaxes are
compiled in (JPEG / JPEG-LS / JPEG 2000 / HTJ2K / JPEG-XL / RLE / deflated / video)
— no external plugins — and ALL character sets are decoded by the native charset
engine (strong on CJK / ISO-2022 multi-byte), which is fault-tolerant: an unknown
or non-conformant vendor SpecificCharacterSet falls back gracefully instead of
erroring. So the read/inspect/convert tools work on real-world, vendor-quirky files.
...
_DIMSE_BIND_PROPS = {
    "bind_address": {
        "description": ("Local source IP for outbound DIMSE on multihomed/VPN hosts. "
                        "String form uses OS-assigned source port; array/object form may "
                        "include an explicit source port."),
        "oneOf": [
            {"type": "string"},
            {"type": "array", "items": {"oneOf": [{"type": "string"}, {"type": "integer"}]},
             "minItems": 1, "maxItems": 2},
            {"type": "object",
             "properties": {"host": {"type": "string"}, "address": {"type": "string"},
                            "ip": {"type": "string"}, "port": {"type": "integer"}}},
        ],
    },
    "bind_host": {"type": "string",
                  "description": "Alias for bind_address host/IP (CLI-style)."},
    "bind_port": {"type": "integer",
                  "description": "Local source TCP port for bind_host; default 0 = OS assigned."},
}

_DIMSE_PEER_PROPS = {
    "host": {"type": "string"},
    "port": {"type": "integer"},
    "called_ae": {"type": "string"},
    "calling_ae": {"type": "string"},
    **_DIMSE_BIND_PROPS,
}


def _dimse_bind_address(a: dict):
    raw = a.get("bind_address")
    if raw is not None:
        if isinstance(raw, dict):
            host = raw.get("host") or raw.get("address") or raw.get("ip")
            if not host:
                raise ValueError("bind_address object requires host/address/ip")
            return (host, int(raw.get("port", 0) or 0))
        return raw
    host = a.get("bind_host")
    if not host:
        if a.get("bind_port"):
            raise ValueError("bind_port requires bind_host")
        return None
    return (host, int(a.get("bind_port", 0) or 0))
...
@tool("normalize_volume",
      "Assemble a DICOM series, normalize voxel intensities, and write NIfTI-1. method='zscore' "
      "(mean/std; optional nonzero-only), 'robust' (percentile rescale to [dst_min,dst_max]), or "
      "'ct' (clip to [clip_lo,clip_hi] HU then z-score to mean/std).",
      {"type": "object", "properties": {
          "path": {"type": "string"}, "output": {"type": "string"},
          "method": {"type": "string", "enum": ["zscore", "robust", "ct"]},
          "nonzero": {"type": "boolean"},
          "dst_min": {"type": "number"}, "dst_max": {"type": "number"},
          "clip_lo": {"type": "number"}, "clip_hi": {"type": "number"},
          "mean": {"type": "number"}, "std": {"type": "number"}},
       "required": ["path", "output", "method"]})
def _normalize(a):
    from . import transforms, volume
    v = volume.load_series(a["path"], recursive=a.get("recursive", True))
    m = a["method"]
    if m == "zscore":
        nv = transforms.normalize_zscore(v, nonzero=a.get("nonzero", False))
    elif m == "robust":
        nv = transforms.rescale_robust(v, dst_min=a.get("dst_min", 0.0), dst_max=a.get("dst_max", 255.0))
    elif m == "ct":
        if "clip_lo" not in a or "clip_hi" not in a:
            raise ValueError("method='ct' requires clip_lo and clip_hi (HU window)")
        nv = transforms.normalize_ct(v, a["clip_lo"], a["clip_hi"], a.get("mean", 0.0), a.get("std", 1.0))
    else:
        raise ValueError(f"unknown method {m!r}")
    return json.dumps({"output": nv.to_nifti(a["output"]), "method": m, "shape": list(nv.shape)})


@tool("roi_mask",
      "One RT Structure Set ROI as a boolean mask volume on an image series' grid — the RTSTRUCT "
      "counterpart of reading a Segmentation. One ROI per call: a structure set with twenty ROIs is "
      "gigabytes of masks together, and merging them into a label volume needs a rule for overlapping "
      "voxels that only the caller can choose. Reports contours it could not place rather than "
      "dropping them, and needs `transform` (from read_registration) when the contours and the series "
      "are in different Frames of Reference.",
      {"type": "object", "properties": {
          "rtstruct": {"type": "string", "description": "the RT Structure Set file"},
          "reference": {"type": "string", "description": "the image series directory the mask lands on"},
          "roi": {"type": "integer", "description": "ROI Number (not a list index)"},
          "transform": {"type": "array", "description": "4x4 RTSTRUCT-frame -> series-frame affine, row-major",
                        "items": {"type": "array", "items": {"type": "number"}}},
          "output": {"type": "string", "description": "write the mask here as a .npy; omit for counts only"}},
       "required": ["rtstruct", "reference", "roi"]})
def _roi_mask(a):
    from .rt import roi_mask
    import numpy as np
    transform = np.asarray(a["transform"], float) if "transform" in a else None
    mask, meta = roi_mask(a["rtstruct"], a["reference"], int(a["roi"]), transform=transform)
    out = {"roi_name": meta["roi_name"], "shape": list(mask.shape),
           "set_voxels": int(meta["set_voxels"]), "findings": meta["findings"]}
    if "output" in a:
...
@tool("read_registration",
      "Read a Spatial Registration (66.1) — how one Frame of Reference sits inside another, which is "
      "what a PET/CT fusion or a contour propagation between series needs. Returns every registered "
      "frame and, when from_uid (and optionally to_uid) is given, the 4x4 transform between them in "
      "the M @ [x,y,z,1] convention. A frame this object does not register, an unusable matrix or a "
      "singular composition answers null — never a silent identity, which would place the overlay "
      "wrongly with nothing to show it did. Deformable Registration (66.3) is refused: it carries a "
      "deformation grid, not a matrix.",
      {"type": "object", "properties": {
          "path": {"type": "string", "description": "the Spatial Registration file"},
          "from_uid": {"type": "string", "description": "Frame of Reference to map from"},
          "to_uid": {"type": "string",
                     "description": "Frame of Reference to map to; omit for the object's own frame"}},
       "required": ["path"]})
def _read_registration(a):
    from .registration import read_registration
    r = read_registration(a["path"])
    out = {"frame_of_reference_uid": r.frame_of_reference_uid,
           "incomplete": r.incomplete,
           "items": [{"frame_of_reference_uid": i.frame_of_reference_uid,
                      "matrix": i.matrix.tolist() if i.matrix is not None else None,
                      "matrix_invalid": i.matrix_invalid,
                      "matrix_type": i.matrix_type,
                      "matrix_count": i.matrix_count} for i in r]}
    if "from_uid" in a:
        m = r.transform(a["from_uid"], a.get("to_uid"))
        out["transform"] = m.tolist() if m is not None else None
    return json.dumps(out)


@tool("read_rtstruct",
      "Read a DICOM RT Structure Set: its label, every ROI (number, name, description, "
      "recommended colour, interpreted type, frame of reference) and each ROI's planar contours as "
      "[x,y,z] LPS patient-mm points. Contours whose ROI reference could not be resolved are reported "
      "separately rather than dropped, and RT-domain defects come back as findings. "
      "geometry_only=true reports the ROI table and findings without the coordinates.",
      {"type": "object", "properties": {
          "path": {"type": "string", "description": "the RT Structure Set file"},
          "geometry_only": {"type": "boolean",
                            "description": "validate Contour Data without returning the points"},
          "roi": {"type": "string", "description": "return only this ROI, by name"}},
       "required": ["path"]})
def _read_rtstruct(a):
    from .rt import read_rtstruct
    ss = read_rtstruct(a["path"], coordinates=not a.get("geometry_only", False))
    wanted = a.get("roi")
    rois = [ss[wanted]] if wanted else list(ss)
    return json.dumps({
        "label": ss.label,
        "rois": [{"number": r.number, "name": r.name, "description": r.description,
                  "rgb": list(r.rgb) if r.rgb else None,
                  "interpreted_type": r.interpreted_type,
                  "frame_of_reference_uid": r.frame_of_reference_uid,
                  "contours": [{"geometric_type": c.geometric_type,
                                "usable": c.usable,
                                "points": c.points.tolist()} for c in r.contours]}
                 for r in rois],
        "unbound_contours": len(ss.unbound_contours),
        "findings": ss.findings})


@tool("write_rtstruct",
      "Author a DICOM RT Structure Set from ROI contours over a reference series. Each ROI: a name, "
      "optional rgb [r,g,b] and interpreted_type code, and contours = a list of planar contours, each "
      "an array of [x,y,z] LPS patient-mm points (matched to its slice by position).",
      {"type": "object", "properties": {
          "reference": {"type": "string", "description": "reference series dir/file-list"},
          "output": {"type": "string"}, "label": {"type": "string"},
          "rois": {"type": "array", "items": {"type": "object", "properties": {
              "name": {"type": "string"},
              "rgb": {"type": "array", "items": {"type": "integer"}},
              "interpreted_type": {"type": "integer"},
              "contours": {"type": "array", "items": {"type": "array",
                           "items": {"type": "array", "items": {"type": "number"}}}}}}}},
       "required": ["reference", "output", "rois"]})
def _write_rtstruct(a):
    from .rt import write_rtstruct
    import numpy as np
    rois = []
    for r in a["rois"]:
        rr = {"name": r.get("name", "ROI"),
              "contours": [np.asarray(c, dtype=np.float64) for c in r.get("contours", [])]}
        if "rgb" in r: rr["rgb"] = tuple(r["rgb"])
        if "interpreted_type" in r: rr["interpreted_type"] = r["interpreted_type"]
        rois.append(rr)
    write_rtstruct(a["reference"], rois, label=a.get("label", ""), output=a["output"])
    return json.dumps({"output": a["output"], "rois": [r.get("name") for r in a["rois"]]})


@tool("write_measurement_report",
      "Author a typed TID 1500 Measurement Report SR from a structured `document` — patient/study, "
      "observer, and measurement groups (findings, ROIs, NUM measurements, qualitative evaluations). "
      "Codes are {value,scheme,meaning}; measurements {name,value,unit,...}.",
      {"type": "object", "properties": {
          "document": {"type": "object", "description": "the TID 1500 document (see pydcm.sr.write_measurement_report)"},
          "output": {"type": "string"}},
       "required": ["document", "output"]})
def _write_measurement_report(a):
    from .sr import write_measurement_report
    write_measurement_report(a["document"], output=a["output"])
    return json.dumps({"output": a["output"], "groups": len(a["document"].get("groups", []))})
...
@tool("validate",
      "Full DICOM conformance — one engine, every layer: IOD / module / Type-1&2 / conditional "
      "(nested) + element-level (VR, value multiplicity, enumerated values, value format, pixel "
      "geometry, LUT) + multi-charset / SpecificCharacterSet conformance (strong on CJK / ISO-2022) + "
      "inter-attribute relationships & cross-references + the SR content tree (TID-template). Returns "
      "{severity, tag, keyword, rule, message} ([] = conformant); validate_iod is the IOD-only view, "
      "validate_sr the SR-only one.",
      _PATH)
...
@tool("dsc_perfusion_maps",
      "DSC-MRI perfusion: deconvolve a dynamic-susceptibility series against an arterial input and write "
      "CBF / CBV / MTT / Tmax / TTP as DICOM Parametric Maps. The AIF is either given directly (`aif`) or "
      "measured from an arterial-ROI mask (`artery_mask`, a .npy boolean array). Optional brain `mask` "
      "(.npy) restricts the fit and anchors the Boxerman-Schmainda leakage reference (`leakage`).",
      {"type": "object", "properties": {
          "path": {"type": "string", "description": "dynamic series directory"},
          "output_dir": {"type": "string"},
          "aif": {"type": "array", "items": {"type": "number"},
                  "description": "ΔR2* arterial input curve (length T); use instead of artery_mask"},
          "artery_mask": {"type": "string", "description": ".npy arterial-ROI mask → measure_aif"},
          "mask": {"type": "string", "description": ".npy brain/tissue mask (fit region + leakage reference)"},
          "method": {"type": "string", "enum": ["ssvd", "csvd", "osvd"]},
          "input": {"type": "string", "enum": ["raw", "concentration"]},
          "times_s": {"type": "array", "items": {"type": "number"}, "description": "acquisition times, seconds"},
          "te_s": {"type": "number"}, "leakage": {"type": "boolean"}},
       "required": ["path", "output_dir"]})
def _dsc(a):
    from . import dsc, perfusion
    import numpy as np
    method = a.get("method", "osvd")
    inp = a.get("input", "raw")
    leakage = bool(a.get("leakage", False))
    te = a.get("te_s")
    is_raw = inp in ("raw", "raw_signal", "signal")

    if "aif" in a and "artery_mask" in a:
        raise ValueError("give either aif OR artery_mask, not both")
    if "aif" in a:
        aif = np.asarray(a["aif"], float)
        source = a["path"]                                  # fit_series reads times/te from tags
        times = np.asarray(a["times_s"], float) if "times_s" in a else None
    elif "artery_mask" in a:
        # measure_aif converts ROI signal → ΔR2*, so it needs the RAW signal; with
        # input='concentration' the series is already ΔR2* — pass aif= directly.
        if not is_raw:
            raise ValueError("artery_mask measures the AIF from raw signal; with "
                             "input='concentration' pass aif= directly")
        # Measure the AIF from the ROI — assemble once and reuse for the fit.
        px, paths, vol4d = perfusion.assemble_4d(a["path"])
        times = (np.asarray(a["times_s"], float) if "times_s" in a
                 else perfusion.series_times_s(paths, perfusion.frames_of(vol4d)))
        if times is None:
            raise ValueError("no acquisition times in tags; pass times_s")
        if te is None and is_raw and paths:
            te = perfusion.seq_te(paths[0])
        aif = dsc.measure_aif(px, np.load(a["artery_mask"]),
                              te_s=(te if te is not None else 0.030))
        source = px
    else:
        raise ValueError("provide either aif (direct curve) or artery_mask (ROI to measure)")

    mask = np.load(a["mask"]) if "mask" in a else None
    maps = dsc.fit_series(source, times_s=times, aif=aif, method=method, input=inp,
                          te_s=te, leakage=leakage, mask=mask)
    written = dsc.write_param_maps(a["path"], maps, output_dir=a["output_dir"])
    files = {k: str(v) for k, v in written.items()} if isinstance(written, dict) else None
    return json.dumps({"output_dir": a["output_dir"], "method": method, "leakage": leakage,
                       "params": list(files) if files else None, "files": files})


@tool("tic_readouts",
      "Time-intensity curve readouts over an ROI of a dynamic series: baseline, peak and peak time, "
      "trough, relative enhancement, wash-in and wash-out slopes, signed AUC and rise time. These are "
      "semi-quantitative — no contrast model, no arterial input — so they apply to any time-resolved "
      "acquisition, unlike the DCE fit. A term that is undefined for the curve comes back null, never 0.",
      {"type": "object", "properties": {
          "path": {"type": "string", "description": "dynamic series directory"},
          "roi_mask": {"type": "string", "description": ".npy boolean ROI mask (spatial dims of the series)"},
          "times_s": {"type": "array", "items": {"type": "number"},
                      "description": "acquisition times, seconds; read from the tags when omitted"},
          "n_baseline": {"type": "integer",
                         "description": "leading samples averaged for the baseline; omit to detect or fall back"},
          "rising": {"type": "boolean",
                     "description": "detect the baseline window instead: true for a curve that rises "
                                    "(T1 enhancement), false for one that drops (T2* susceptibility)"},
          "rise_frac": {"type": "number", "description": "fraction of peak enhancement for rise_time (default 0.5)"}},
       "required": ["path", "roi_mask"]})
def _tic(a):
    ...
    if times is None:
        raise ValueError("no acquisition times in tags; pass times_s")
    m = perfusion.tic_roi(px, np.load(a["roi_mask"]), times,
                          n_baseline=a.get("n_baseline"), rising=a.get("rising"),
                          rise_frac=float(a.get("rise_frac", 0.5)))
    # An undefined term is NaN, which JSON cannot spell. null says "no reading";
    # 0 would be a number, and a wrong one.
...
_DEIDENT_OPTS = ("profile", "patient_name", "patient_id", "retain_dates", "retain_safe_private",
                 "retain_uids", "retain_device_id", "retain_institution_id", "retain_patient_chars",
                 "clean_descriptors", "clean_graphics", "clean_struct_content", "clean_pixel",
                 "shift_dates_days", "replace", "remove")
_DEIDENT_SCHEMA = {
    "profile": {"type": "string", "enum": ["basic", "none"], "description": "basic = PS3.15 Annex E scrub"},
    "patient_name": {"type": "string"}, "patient_id": {"type": "string"},
    "retain_dates": {"type": "boolean"}, "retain_safe_private": {"type": "boolean"},
    "retain_uids": {"type": "boolean"}, "retain_device_id": {"type": "boolean"},
    "retain_institution_id": {"type": "boolean"}, "retain_patient_chars": {"type": "boolean"},
    "clean_descriptors": {"type": "boolean"}, "clean_graphics": {"type": "boolean"},
    "clean_struct_content": {"type": "boolean"},
    "clean_pixel": {"type": "boolean", "description": "113101 — black out burned-in annotation"},
    "shift_dates_days": {"type": "integer"},
    "replace": {"type": "object", "description": "{tag: value} per-tag overrides"},
    "remove": {"type": "array", "items": {"type": "string"}, "description": "tags to erase"}}


@tool("deidentify_file",
      "De-identify a DICOM file per PS3.15 Annex E (Basic Profile) → new internally-consistent UIDs + "
      "de-identification method codes (CID 7050). EVERY Table E.1-1 option column is exposed: retain "
      "dates / safe-private / UIDs / device-id / institution-id / patient-characteristics; clean "
      "descriptors / graphics / structured-content; and clean_pixel (113101, black out burned-in "
      "annotation). shift_dates_days offsets Modified-Dates; replace/remove apply per-tag edits.",
      {"type": "object",
       "properties": {"path": {"type": "string"}, "output": {"type": "string"}, **_DEIDENT_SCHEMA},
       "required": ["path", "output"]})
def _deident(a):
    from .deident import deidentify
    kw = {k: a[k] for k in _DEIDENT_OPTS if k in a}
    with open(a["path"], "rb") as f:
        out = deidentify(f.read(), **kw)
    with open(a["output"], "wb") as f:
        f.write(out)
    return json.dumps({"output": a["output"], "profile": a.get("profile", "basic")})
...
def _dimse_store(a):
    from . import dimse
    ae = dimse.AE(ae_title=a.get("calling_ae", "PYDCM"))
    ae.add_requested_contexts_for_files(a["files"])
    assoc = ae.associate(a["host"], int(a["port"]), ae_title=a.get("called_ae", "ANY-SCP"),
                         bind_address=_dimse_bind_address(a))
    try:
        if not assoc.is_established:
            return json.dumps({"established": False})
        r = assoc.send_c_store_files(a["files"])
        return json.dumps({"established": True, **r})
    finally:
        assoc.release()
...
@tool("dicomweb_search",
      ...
      {"type": "object", "properties": {
          ...
          "base_path": {"type": "string", "description": "DICOMweb path prefix, e.g. '/dicom-web' for "
                        "Orthanc/dcm4chee; '' for a root-mounted server. May instead be appended to `server`."},
          "auth": {"type": "string"}},
       "required": ["server"]})
def _dwq(a):
    from . import dicomweb
    level = a.get("level", "studies")
    kw = {"matches": a.get("matches"), "auth": a.get("auth", ""), "base_path": a.get("base_path", "")}
    if level == "studies":
...
@tool("dicomweb_retrieve",
      ...
      {"type": "object", "properties": {
          ...
          "base_path": {"type": "string", "description": "DICOMweb path prefix, e.g. '/dicom-web' for "
                        "Orthanc/dcm4chee; '' for a root-mounted server. May instead be appended to `server`."},
          "transfer_syntax": {"type": "string", "description": "WADO-RS transfer-syntax: omitted = "
                              "server default (PS3.18: Explicit VR LE, may transcode); '*' = stored "
                              "encoding verbatim (no transcode — keep compressed as-is); a TS UID = "
                              "that encoding."},
          "auth": {"type": "string"}},
...
def _dwr(a):
    ...
    parts = dicomweb.retrieve_study(a["server"], a["study_uid"],
                                    base_path=a.get("base_path", ""),
                                    transfer_syntax=a.get("transfer_syntax", ""),
                                    auth=a.get("auth", ""))
    paths = []
...
def _dws(a):
    ...
    res = dicomweb.store_instances(a["server"], blobs,
                                   base_path=a.get("base_path", ""), auth=a.get("auth", ""))
    return json.dumps(res)
...
@tool("render_markup",
      "Render a DICOM frame to PNG with structured markup BURNED IN — GSPS annotations (in their layer "
      "colour) and SR SCOORD measurement regions (green) that reference this image — so an agent SEES "
      "where each measurement / annotation lies on the pixels. describe=true also returns the projected "
      "markup as JSON (per shape: source, kind, pixel points, colour, text / measurement value). "
      "Requires Pillow.",
      {"type": "object", "properties": {
          "path": {"type": "string", "description": "the DICOM image"},
          "overlays": {"type": "array", "items": {"type": "string"},
                       "description": "GSPS / SR file paths whose markup references the image"},
          "output": {"type": "string"}, "frame": {"type": "integer", "description": "1-based (default 1)"},
          "window_center": {"type": "number"}, "window_width": {"type": "number"},
          "describe": {"type": "boolean"}},
       "required": ["path", "output"]})
def _render_markup(a):
    from .overlay import render_overlay
    try:
        from PIL import Image
    except ImportError:
        return json.dumps({"error": "render_markup requires Pillow (pip install pillow)"})
    win = (float(a.get("window_center", 0.0)), float(a["window_width"])) if a.get("window_width") else None
    describe = bool(a.get("describe", False))
    res = render_overlay(a["path"], a.get("overlays", []), frame=int(a.get("frame", 1)),
                         window=win, with_overlays=describe)
    rgb, marks = res if describe else (res, None)
    Image.fromarray(rgb).save(a["output"])
    out = {"output": a["output"], "height": int(rgb.shape[0]), "width": int(rgb.shape[1]),
           "overlays_drawn": len(a.get("overlays", []))}
    if describe:
        out["markup"] = marks
    return json.dumps(out, default=str)
...
@tool("deidentify_series",
      "De-identify every DICOM file under a directory through ONE session (consistent UID remap across "
      "the set, so intra-study cross-references still resolve) → output directory. Accepts the SAME "
      "full PS3.15 Table E.1-1 option set as deidentify_file (retain/clean columns + clean_pixel).",
      {"type": "object",
       "properties": {"path": {"type": "string"}, "output_dir": {"type": "string"},
                      "recursive": {"type": "boolean"}, **_DEIDENT_SCHEMA},
       "required": ["path", "output_dir"]})
...
@tool("clean_burned_in_pixels",
      "Erase burned-in PHI TEXT printed into the image pixels (patient name / dates / annotation "
      "overlays) and write the cleaned file. use_ctp (default true) auto-detects and blacks out the "
      "known burned-in regions via the RSNA CTP device-signature library (matched by manufacturer / "
      "model); pass explicit `regions` boxes for anything else. require_match raises if nothing matches "
      "and no regions are given (fail-closed). Output is re-emitted uncompressed — pixel quality intact.",
      {"type": "object", "properties": {
          "path": {"type": "string"}, "output": {"type": "string"},
          "regions": {"type": "array", "items": {"type": "array", "items": {"type": "integer"}},
                      "description": "[(x, y, w, h) or (x, y, w, h, frame), …] always-blacked boxes"},
          "use_ctp": {"type": "boolean"}, "require_match": {"type": "boolean"}},
       "required": ["path", "output"]})
def _clean_px(a):
    from . import clean_pixel_data
    kw = {k: a[k] for k in ("regions", "use_ctp", "require_match") if k in a}
    with open(a["path"], "rb") as f:
        out = clean_pixel_data(f.read(), **kw)
    with open(a["output"], "wb") as f:
        f.write(out)
    return json.dumps({"output": a["output"], "ctp": a.get("use_ctp", True)})


@tool("wsi_read_region",
      "Read a rectangular region from a DICOM whole-slide-image pyramid and write it as PNG — the "
      "byte-range viewer read: only the tiles covering the region are fetched & decoded (not the whole "
      "slide). location (x,y) is in LEVEL-0 pixels; size (w,h) is in the chosen level's pixels. "
      "Requires Pillow.",
      {"type": "object", "properties": {
          "path": {"type": "string"}, "output": {"type": "string"},
          "x": {"type": "integer"}, "y": {"type": "integer"}, "level": {"type": "integer"},
          "w": {"type": "integer"}, "h": {"type": "integer"}},
       "required": ["path", "output", "x", "y", "level", "w", "h"]})
def _wsi_region(a):
    from .wsi import open_slide
    try:
        from PIL import Image
    except ImportError:
        return json.dumps({"error": "wsi_read_region requires Pillow (pip install pillow)"})
    s = open_slide(a["path"])
    arr = s.read_region((int(a["x"]), int(a["y"])), int(a["level"]),
                        (int(a["w"]), int(a["h"])), rgba=False)
    Image.fromarray(arr).save(a["output"])
    return json.dumps({"output": a["output"], "level": int(a["level"]),
                       "size": [int(a["w"]), int(a["h"])], "levels": s.level_count})
...
@tool("transcode",
      "Re-encode a DICOM file to another transfer syntax. Compress to any BUILT-IN lossless codec "
      "(RLE / JPEG 2000 / JPEG-LS / HTJ2K / JPEG-XL) or decompress to uncompressed (implicit/explicit "
      "VR LE, explicit BE). All codecs are compiled into the one wheel — NO external plugins; on input "
      "the decoder reads every encoded transfer syntax (JPEG / JPEG-LS / J2K / HTJ2K / RLE / deflated).",
      {"type": "object", "properties": {
          "path": {"type": "string"}, "output": {"type": "string"},
...
def _dwdel(a):
    from . import dicomweb
    lvl, auth, bp = a.get("level", "study"), a.get("auth", ""), a.get("base_path", "")
    if lvl == "study":
        st = dicomweb.delete_study(a["server"], a["study_uid"], base_path=bp, auth=auth)
    elif lvl == "series":
        st = dicomweb.delete_series(a["server"], a["study_uid"], a["series_uid"], base_path=bp, auth=auth)
    else:
        st = dicomweb.delete_instance(a["server"], a["study_uid"], a["series_uid"], a["instance_uid"], base_path=bp, auth=auth)
...
def _dwmeta(a):
    ...
    bp = a.get("base_path", "")
    if a.get("series_uid"):
        r = dicomweb.retrieve_series_metadata(a["server"], a["study_uid"], a["series_uid"], base_path=bp, auth=a.get("auth", ""))
    else:
        r = dicomweb.retrieve_study_metadata(a["server"], a["study_uid"], base_path=bp, auth=a.get("auth", ""))
    return _dump(r)
...
def _dwrend(a):
    ...
    data = dicomweb.retrieve_rendered(a["server"], a["study_uid"], a.get("series_uid", ""),
                                      a.get("instance_uid", ""), base_path=a.get("base_path", ""),
                                      window=a.get("window"), auth=a.get("auth", ""))
    with open(a["output"], "wb") as f:
        f.write(data)
...
@tool("dimse_get",
      ...
      {"type": "object", "properties": {
          **_DIMSE_PEER_PROPS,
          ...
          "level": {"type": "string", "enum": ["PATIENT", "STUDY", "SERIES", "IMAGE"]},
          "matches": {"type": "object"}, "model": {"type": "string", "enum": ["study", "patient"]},
          "store_classes": {"type": "array", "items": {"type": "string"},
                            "description": "Storage SOP Class UIDs to receive (from a prior "
                            "dimse_find) — declared classes negotiate their full transfer-syntax "
                            "set so a non-transcoding SCP can return compressed instances; "
                            "omitted = the common image/report classes."}},
...
def _dimse_get(a):
    ...
    def handle_store(event):
        ds = event.dataset                       # already carries its received file_meta + pixels
        p = os.path.join(a["output_dir"], f"{ds.SOPInstanceUID}.dcm")
        ds.save_as(p, write_like_original=False)  # byte-verbatim, preserves the stored TS
        saved.append(p)
        return 0x0000

    ae = dimse.AE(ae_title=a.get("calling_ae", "PYDCM"))
    ae.add_requested_context(model)
    # Declare the Storage classes to receive (the recommended C-GET path — add_supported_context
    # negotiates the scp_role contexts with their full transfer-syntax set). Declared classes (from
    # a prior dimse_find) let a non-transcoding SCP return compressed instances; omitted falls back
    # to the common image/report set — same model and curated source as the CLI get-scu.
    for cls in (a.get("store_classes") or dimse._COMMON_STORAGE):
    ...
    for k, v in (a.get("matches") or {}).items():
        setattr(q, k, v)
    assoc = ae.associate(a["host"], int(a["port"]), ae_title=a.get("called_ae", "ANY-SCP"),
                         evt_handlers=[(dimse.evt.EVT_C_STORE, handle_store)],
                         bind_address=_dimse_bind_address(a))
    try:
        if not assoc.is_established:
            return json.dumps({"established": False})
