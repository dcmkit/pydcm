...
def content(path, contours=False, control_points=False):
    """Semantic content of a structured DICOM object — Segmentation, RT Structure
    Set, RT Plan, RT Dose, Presentation State, Waveform, Ophthalmic Visual Field, or
    Structured Report (the full content tree) — as a dict (coded concepts resolved),
    or ``None`` if `path` is not one of those.
