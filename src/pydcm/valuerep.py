...
``format_number_as_ds`` — with the usual ``isinstance`` semantics and
``from pydcm.valuerep import VR`` imports.
"""
...
# subtypes so isinstance / `from pydcm.valuerep import DA` keep working.
class DA(str):
