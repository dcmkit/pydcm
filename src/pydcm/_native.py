...
def transcode(part10: bytes, target_ts: str) -> bytes:
    """Re-encode Part-10 bytes to ``target_ts`` (encapsulated lossless: JPEG2000 .90 /
    JPEG-LS .80 / HTJ2K .201 / JPEG-XL). Backs ``Dataset.compress``."""
    return require().transcode(bytes(part10), str(target_ts))
