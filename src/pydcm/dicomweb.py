...
def _split_server_prefix(server, base_path=""):
    """Fold any path embedded in ``server`` into ``base_path``.

    The native transport addresses a server by ``scheme://host:port`` only, so a DICOMweb prefix
    pasted into ``server`` (e.g. ``"http://host:8042/dicom-web"``) would otherwise be dropped and
    the request would hit the server's NATIVE API (Orthanc → bare resource IDs) instead of
    DICOMweb. Splitting it here makes either spelling equivalent::

        search_studies("http://host:8042/dicom-web")
        search_studies("http://host:8042", base_path="/dicom-web")
    """
    parts = urlsplit(str(server))
    prefix = parts.path.rstrip("/")
    if prefix:
        return (urlunsplit((parts.scheme, parts.netloc, "", "", "")),
                prefix + str(base_path or ""))
    return str(server), str(base_path or "")


def _honor_server_prefix(fn):
    """Wrap a ``(server, …, base_path=…)`` entry point so a prefix in ``server`` is honored,
    not silently dropped. Only overrides ``base_path`` when ``server`` actually carried a path,
    so a function's own default (e.g. WADO-URI's ``/wado``) is preserved otherwise."""
    @functools.wraps(fn)
    def wrapper(server, *args, **kwargs):
        norm, prefix = _split_server_prefix(server, kwargs.get("base_path", ""))
        if norm != str(server):
            kwargs["base_path"] = prefix
        return fn(norm, *args, **kwargs)
    return wrapper
...
def resolve_auth(config) -> tuple[str, str]:
...
def start_retrieve(server, study_uid, series_uid="", *, store_dir, base_path="",
    ...
    return _dicomweb.start_retrieve(str(server), str(base_path or ""), level, str(study_uid),
                                    str(series_uid or ""), str(store_dir),
...
def search_workitems(server, *, base_path="", matches=None, includefields=None,
    ...
    """
    wire_limit = None if limit is None else int(limit)
    wire_offset = int(offset)
    if wire_limit is not None and not 0 <= wire_limit <= 0xFFFFFFFF:
        raise ValueError("limit must be in the uint32 range")
    if not 0 <= wire_offset <= 0xFFFFFFFF:
        raise ValueError("offset must be in the uint32 range")
    r = _dicomweb.ups_search(str(server), str(base_path or ""),
                             {str(k): str(v) for k, v in (matches or {}).items()},
                             [str(f) for f in (includefields or [])],
                             wire_limit, wire_offset, str(auth or ""))
...
def change_workitem_state(server, workitem_uid, state, transaction_uid, *, requester="",
                          base_path="", auth=""):
    ...
    return _dicomweb.ups_change_state(str(server), str(base_path or ""), str(workitem_uid),
                                      str(state), str(transaction_uid), str(requester or ""),
                                      str(auth or ""))


def request_cancel_workitem(server, workitem_uid, *, reason=None, requester="",
                            base_path="", auth=""):
    ...
    return _dicomweb.ups_request_cancel(str(server), str(base_path or ""), str(workitem_uid),
                                        _json_body(reason), str(requester or ""), str(auth or ""))


def subscribe_workitem(server, workitem_uid, ae_title, *, deletion_lock=False, filters=None,
                       base_path="", auth=""):
    ...
    return _dicomweb.ups_subscribe(str(server), str(base_path or ""), str(workitem_uid),
                                   str(ae_title),
                                   {str(k): str(v) for k, v in (filters or {}).items()},
                                   bool(deletion_lock), str(auth or ""))
...
def suspend_global_subscription(server, global_resource_uid, ae_title, *, base_path="", auth=""):
    """UPS-RS suspend a global or filtered-global subscription."""
    return _dicomweb.ups_suspend(str(server), str(base_path or ""),
                                 str(global_resource_uid), str(ae_title), str(auth or ""))
...
__all__ = ['search_studies', 'search_series', 'search_instances', 'search_workitems',
           ...
           'suspend_global_subscription', 'resolve_auth']

# Honor a DICOMweb prefix embedded in `server` for every entry point whose first parameter is
# `server` — so a pasted "http://host:8042/dicom-web" works as well as the (server, base_path=…)
# split, instead of silently dropping the prefix to the server's native API. retrieve_bulkdata
# (first param `uri`) is left alone.
for _name, _fn in list(globals().items()):
    if (inspect.isfunction(_fn) and not _name.startswith("_")
            and next(iter(inspect.signature(_fn).parameters), None) == "server"):
        globals()[_name] = _honor_server_prefix(_fn)
del _name, _fn
