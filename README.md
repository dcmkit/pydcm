...
| Diffusion tensor (DTI) | `pydcm.dti` | tensor fit from a DWI series → FA / MD / AD / RD / DEC / Westin maps (vendor-aware baseline split, unit-vector guard, head masking) + deterministic RK4 tractography → streamlines in patient mm, written back as Tractography Results |
...
| RT dose &amp; structures | `pydcm.rt` | `read_rtdose` / `write_rtdose` / `read_rtplan` / `dose_at` (dose in Gy at arbitrary patient-frame points, with an inside-grid flag) / `dvhcalc` (full ROI DVH coverage, with `D95` / `D2cc` / `V20Gy` / `V100%` constraint readings) + `write_rtstruct` / `read_rtstruct` / `roi_mask` (RTSTRUCT ROI → boolean volume on a series' grid, cross-frame via `read_registration`) — author AND read RT Structure Sets (ROI table, coded semantics, contours as `(n,3)` LPS arrays; contours that cannot be placed are reported, never dropped) |
| Tractography | `write_mktract` / `read_tract` | author **and** read Tractography Results (66.6) — streamlines in Frame-of-Reference world mm + per-track FA / ADC measurements / statistics |
| Perfusion | `pydcm.dce` / `pydcm.dsc` / `pydcm.perfusion` | DCE-MRI pharmacokinetic fit (Tofts / extended Tofts / Patlak, Parker or measured AIF), DSC-MRI deconvolution (sSVD / cSVD / oSVD, Boxerman-Schmainda leakage) → CBF / CBV / MTT / Tmax, and model-free time-intensity readouts (`perfusion.tic` — peak, time-to-peak, enhancement, wash-in / wash-out, signed AUC, rise time). Parameter maps written back as DICOM Parametric Maps |
| Spatial registration | `pydcm.registration` | read **and** author Spatial Registration (66.1) — `read_registration` / `write_registration` for the 4x4 transform between two Frames of Reference, which is what a PET/CT fusion or a cross-series contour propagation needs. An unregistered frame or an unusable matrix answers `None`, never a silent identity |
| Tumour response | `pydcm.recist` | RECIST 1.1 / iRECIST / mRECIST / Cheson-Lugano / RANO / PCWG3 (nadir-referenced) + Feret long / short-axis diameter and volume measured off a SEG mask + cross-study lesion matching in patient coordinates |
...
| SEG | `write_seg` / `write_seg_fractional` / `write_seg_overlapping` / `read_seg` | coded SEG, binary + fractional + overlapping (hierarchical labels), SEG → labelmap. The writers take the segmentation's own SOP / Series / Dimension-Organization UIDs and content timestamps, so repeated runs and separate producers stay distinct instances |
...
| SR / TID 1500 | `write_sr` / `write_report` / `read_report` / `read_regions` / `sr_validate` | a full SR object model (Comprehensive / Enhanced / 3D SR, ROI measurement groups, every content-item type) — author from a content tree or as TID 1500 reports, read them back (including a region-first `read_regions` that returns every SCOORD / SCOORD3D and segment reference, 3-D POLYGON / ELLIPSOID included), validate structure + PS3.16 coded concepts, render SR → HTML / JSON |
| KO / PR / annotations | `write_ko` / `write_pr` / `read_ann` | Key Object Selection (with the XDS-I retrieve hints a published manifest needs), Presentation State, Bulk Annotations. The authoring writers take `sop_instance_uid` / `series_instance_uid`, so a producer minting many documents is not stuck with the engines' deterministic default |
| Encapsulated documents | `write_encapsulated` / `read_encapsulated` | PDF / CDA / STL / OBJ / MTL (PS3.3 A.104) |
| Surface meshes | `read_surface` / `write_surface` | read **and** author Surface Segmentation (66.5) — `(points, triangles)` round-trip, coded segments, every mesh primitive (triangle / strip / fan / facet) as `(N,3)` / `(M,3)` arrays |
...
plan = pydcm.read_rtplan("rtplan.dcm")     # prescription + fractionation
dvh = pydcm.dvhcalc("rtstruct.dcm", "rtdose.dcm", roi_number, rx_dose=plan)

dvh.D95, dvh.D2cc, dvh.V20Gy               # constraints, written as they are written
dvh.statistic("V100%")                     # ("%" is not an identifier — call it by name)
dvh.dose_constraint(95)                    # D(V) in Gy — the same reading, called directly
dvh.volume_constraint(20)                  # V(D) in cm3
dvh.describe()                             # the plan-review summary

pydcm.write_rtdose(dose, affine=grid.affine, output="out.dcm")  # conformance-clean
```

The D(V) / V(D) bin search is the native engine's, not this wrapper's — the same
the DVH constraint grammar accepts, so a D95 is one number across both.
A constraint the structure is too small to answer is `nan` (`null` in the CLI's
JSON), never 0 Gy.
...
pydcm.write_seg_overlapping("ct/", masks, segments, output="seg.dcm")  # segments MAY overlap
...
`pydcm.mcp` exposes **90 tools** over the live engine — *more* than the command-line
