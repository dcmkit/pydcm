...
4. **`generate_uid()` mints under pydcm's own registered root** —
   `1.2.826.0.1.3680043.9.7155.` (`PYDCM_ROOT_UID`), so the UIDs pydcm creates
   carry its own identity. UIDs are opaque unique strings, so this is transparent
   in practice; pass an explicit `prefix=` for a different root, or `prefix=None`
   for a `2.25.` UUID-derived UID.
...
## RT: two things worth knowing

- **`DVH.counts` and `DVH.cumulative` are both arrays, always.** pydcm computes
  the differential histogram and its suffix-sum together and hands you both, so
  neither attribute is a DVH object you have to convert. `bins` /`bincenters`
  are the dose axis in Gy.

- **A constraint with no answer is `nan`, not `0 Gy`.** `dvh.D2cc` on a 0.5 cm³
  structure asks about volume that is not there. NaN says so; zero would be a
  dose, and a wrong one. The CLI writes the same reading as JSON `null`.
