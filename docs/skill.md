...
`pydcm` is one wheel that decodes **every** transfer syntax and reads **every**
character set, then carries a workflow the whole way — volumes, NIfTI, SEG/RT/SR,
...
- **In-process MCP** (`python -m pydcm.mcp`, ~79 tools) — for tool-calling
...
| Validate (IOD + SR conformance) | `pydcm.iod_validate`, `pydcm.sr_validate` |
...
pydcm.iod_validate("pred.dcm")                      # validate authored objects before egress
```
...
- **Validate authored objects** (`iod_validate`, `sr_validate`) before writing
  SEG / RT / SR / parametric maps out.
...
- **As an MCP server**: `python -m pydcm.mcp` (≈79 tools) — see
