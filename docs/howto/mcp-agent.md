...
The server covers pydcm end to end — 90+ tools, grouped here by job. Each
...
| `dicom_content` | Semantic JSON of a structured object (SEG / RT Struct / RT Plan / RT Treatment Record / RT Dose / PS / Waveform / OPV / SR content tree) |
...
*Connecting the networking tools.* The DIMSE tools take `host` / `port` / `called_ae`; the
DICOMweb tools take `server` (origin) + `base_path` + `auth`. On multihomed or VPN hosts,
DIMSE tools normally use the OS-selected source address; pass `bind_address` (or
`bind_host` + optional `bind_port`) only when a peer allow-list requires a specific local IP.
The `base_path` differs per product — pass it explicitly:

```jsonc
// dicomweb_search against Orthanc
{ "server": "http://localhost:8042", "base_path": "/dicom-web",
  "level": "studies", "auth": "Basic b3J0aGFuYzpvcnRoYW5j" }

// dicomweb_search against dcm4chee-arc
{ "server": "http://localhost:8080", "base_path": "/dcm4chee-arc/aets/DCM4CHEE/rs",
  "level": "studies" }

// dimse_find against Orthanc (DIMSE port 4242, AE ORTHANC)
{ "host": "localhost", "port": 4242, "called_ae": "ORTHANC",
  "matches": { "QueryRetrieveLevel": "STUDY", "PatientID": "" } }

// dimse_echo from a specific VPN/source IP
{ "host": "pacs.local", "port": 104, "called_ae": "PACS",
  "bind_address": ["10.8.0.23", 0] }

// dimse_get — retrieve a study, declaring the Storage classes the find above showed
{ "host": "localhost", "port": 4242, "called_ae": "ORTHANC", "output_dir": "./out",
  "level": "STUDY", "matches": { "StudyInstanceUID": "1.2.." },
  "store_classes": ["1.2.840.10008.5.1.4.1.1.2"] }
```

A root-mounted server uses `"base_path": ""`. HTTPS: pass an `https://` origin and trust a private
CA via the `SSL_CERT_FILE` env var on the server process.

For `dimse_get`, declare the study's Storage classes in `store_classes` (the SOP Class UIDs the
prior `dimse_find` returned) so a non-transcoding server can return compressed instances — it aborts
the retrieval on any matched class without a negotiated context. Omitted ⇒ the common image and
report classes.
