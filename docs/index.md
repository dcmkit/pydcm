...
| RT dosimetry | `pydcm.rt`, `dvhcalc` | dose read / write + DVH with full ROI coverage |
...
| Semantic content | `pydcm.content` | one reader for SEG / RT / Presentation State / Waveform / Ophthalmic Visual Field / Structured Report → structured JSON |
...
| Agent / MCP server | `pydcm.mcp` | in-process MCP — **70+ tools spanning the whole toolkit** for an LLM agent |
